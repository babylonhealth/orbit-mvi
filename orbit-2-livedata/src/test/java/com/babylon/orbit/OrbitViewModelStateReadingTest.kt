...
import com.babylon.orbit.v2.BogusDependency
import com.babylon.orbit.v2.MyClass
...
class OrbitViewModelStateReadingTest {
    ...
    fun `newDSLTestTest`() {
        ...
        val testSubject = MyClass(mockDependency).test()
        val spy = spy(testSubject)

//        doAnswer {  }.whenever(spy).somethingElse(anyString())
        // The below does not work with spies
//        whenever(spy.somethingElse(anyString())).thenAnswer {  }
        spy.something(true)

        verify(spy, times(5)).somethingElse(any())
    }
